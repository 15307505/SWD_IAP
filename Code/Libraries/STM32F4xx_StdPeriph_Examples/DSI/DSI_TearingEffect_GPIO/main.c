/**
  ******************************************************************************
  * @file    DSI_TearingEffect_GPIO/main.c 
...
int main(void)
{
  ...
  CmdCfgStructure.ColorCoding           = DSI_RGB565;
  ...
  /* invert colors */
  Delay(3000);
  DSI_ShortWrite(DSI, 0, DSI_DCS_SHORT_PKT_WRITE_P0, DSI_ENTER_INVERT_MODE, 0);
  
  /* restore colors */
  Delay(3000);
  DSI_ShortWrite(DSI, 0, DSI_DCS_SHORT_PKT_WRITE_P0, DSI_EXIT_INVERT_MODE, 0);
  
  /* enter idle mode */
  Delay(3000);
  DSI_ShortWrite(DSI, 0, DSI_DCS_SHORT_PKT_WRITE_P0, DSI_ENTER_IDLE_MODE, 0);
  
  /* exit idle mode */
  Delay(3000);
  DSI_ShortWrite(DSI, 0, DSI_DCS_SHORT_PKT_WRITE_P0, DSI_EXIT_IDLE_MODE, 0);
  
  /* enter partial mode */
  Delay(3000);
  DSI_LongWrite(DSI, 0, DSI_DCS_LONG_PKT_WRITE, 4, DSI_SET_PARTIAL_ROWS, reduced_table_row);
  DSI_ShortWrite(DSI, 0, DSI_DCS_SHORT_PKT_WRITE_P0, DSI_ENTER_PARTIAL_MODE, 0);
  
  /* enter normal mode */
  Delay(3000);
  DSI_ShortWrite(DSI, 0, DSI_DCS_SHORT_PKT_WRITE_P0, DSI_ENTER_NORMAL_MODE, 0);
  
  /* RB swap (BGR) */
  Delay(3000);
  DSI_ShortWrite(DSI, 0, DSI_DCS_SHORT_PKT_WRITE_P1, DSI_SET_ADDRESS_MODE, 0x8);
  
  /* regular RGB */
  Delay(3000);
  DSI_ShortWrite(DSI, 0, DSI_DCS_SHORT_PKT_WRITE_P1, DSI_SET_ADDRESS_MODE, 0x0);
  
  /* Y */
  Delay(3000);
  DSI_ShortWrite(DSI, 0, DSI_DCS_SHORT_PKT_WRITE_P1, DSI_SET_ADDRESS_MODE, 0x80);
  
  /* XY */
  Delay(3000);
  DSI_ShortWrite(DSI, 0, DSI_DCS_SHORT_PKT_WRITE_P1, DSI_SET_ADDRESS_MODE, 0xC0);
  
  /* X */
  Delay(3000);
  DSI_ShortWrite(DSI, 0, DSI_DCS_SHORT_PKT_WRITE_P1, DSI_SET_ADDRESS_MODE, 0x40);
...
static void DSI_Config(void)
{
  ...
  GPIO_PinAFConfig(GPIOJ, GPIO_PinSource2, GPIO_AF_DSI);  
  ...
  RCC_PLLSAIConfig(LTDC_PLLSAIN, LTDC_PLLSAIP, LTDC_PLLSAIQ, LTDC_PLLSAIR);
  
  /* LTDC clock frequency = PLLLCDCLK / RCC_PLLSAIDivR_Div2 = 83.4 / 2 = 41.7 MHz */  
  ...
  NVIC_InitStructure.NVIC_IRQChannel = DSI_IRQn;
