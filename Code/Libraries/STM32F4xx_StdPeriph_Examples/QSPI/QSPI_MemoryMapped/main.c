...
uint8_t aTxBuffer[] = "QSPI MemoryMapped Example : QSPI -> External QSPI Nor Flash";
...
int main(void)
{
  /*!< At this stage the microcontroller clock setting is already configured, 
     ...
     */
  uint32_t address = 0, index = 0x00, step = 0x00, ubRxIndex = 0x00;
...
/* Workaround for DMA2 Limitation */
  ...
  while(1)
  {
    ...
    case 4:
      ...
      pAddr = (uint8_t*) qspiaddr + address;
      while(ubRxIndex < BUFFERSIZE)
      {
        aRxBuffer[ubRxIndex++] = *pAddr++;
      }
      QSPI_AbortRequest();
      QSPI_Cmd(DISABLE);
    ...
    case 5:
      ...
      if((QSPI_START_EXT_ADDR + address) >= QSPI_END_EXT_ADDR)
      {
        address = 0;
      }
      step = 0;
